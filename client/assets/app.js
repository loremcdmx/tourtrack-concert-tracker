...
async function clearArtistCache() {
  try {
    ...
    alert('Cache cleared — next scan will re-fetch everything.');
...
function saveGame() {
  if (!concerts.length && !festivals.length) {
    alert('Nothing to save — run a scan first.');
    return;
  }
  const now = new Date();
  ...
  const filename = `tourtrack_${now.toISOString().slice(0,10)}_${now.getHours()}h${String(now.getMinutes()).padStart(2,'0')}.tt`;
...
function loadGameFile(ev) {
  ...
  reader.onload = e => {
    try {
      ...
      if (!data._tt) throw new Error('Not a TourTrack save file');
...
function resetForNewScan() {
  const ok = confirm(
    ...
    'Consider saving first (💾 Save current state).'
  );
  if (!ok) return;
  ...
  closeSaveLoad();
  setStatus('Data reset — starting fresh scan…', false);
...
function purgePastConcerts() {
  ...
  if (!pastCount) {
    alert('No past concerts to remove — everything is upcoming.');
    return;
  }
...
function quickReset() {
  ...
  setStatus('Reset — ready for a fresh scan', false);
...
function newGame() {
  ...
  renderOnboardHistory && renderOnboardHistory();
  setStatus('New game — ready for a fresh scan', false);
...
function openGoThru() {
  ...
  if (!upFests.length) {
    alert('No festivals match the current date / location filters.');
    return;
  }
...
function saveAndFetch(forceRefresh = false) {
  ...
  if (!API_KEY) { alert('Enter your Ticketmaster API key.'); return; }
  const parsed = parseArtistLines(document.getElementById('artists-ta').value);
  if (!parsed.length) { alert('Add at least one artist.'); return; }
...
function refreshKeyPoolUI() {
  ...
  TM_KEYS.forEach((k, i) => {
    ...
    del.onclick = () => {
      if (TM_KEYS.length <= 1) { alert('Keep at least one API key.'); return; }
      TM_KEYS.splice(i, 1);
...
function addKeyToPool() {
  ...
  if (!key) return;
  if (TM_KEYS.some(k => k.key === key)) { alert('This key is already in the pool.'); return; }
...
function quotaApplyNewKey() {
  if (SERVER_MANAGED_TICKETMASTER) {
    alert('Ticketmaster keys are managed by the server for this deployment.');
    return;
  }
...
async function onboardImport() { return runSpotifyImport({ mode: 'onboard' }); }
async function spotifyImport()  { return runSpotifyImport({ mode: 'settings' }); }
...
if (concerts.length || festivals.length) {
  ...
  checkIDBCache().then(info => {
    if (info && info.artistCount > 0) {
      ...
      setStatus('Paste your playlist to get started', false);
      renderOnboardHistory();
      const skip = document.getElementById('onboard-skip');
